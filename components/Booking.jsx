import React from 'react'

const Booking = () => {
  return (
    <div id="#booking" className="bg-transparent backdrop-blur-2xl mt-20 flex max-w-3xl h-[700px] m-auto overflow-hidden shadow-sm font-primary drop-shadow-xl/50 rounded-sm">
    <div className="relative w-full bg-6">
        <div className="absolute inset-0 flex flex-col items-center justify-center text-gray-800/90 text-shadow-lg/80 text-shadow-white text-6xl">
        <div className="absolute inset-0  " />
            <h2 className="z-10 m-10 ">Travel / Booking Information:</h2>
            <h2 className="z-10 m-10">El Conquistador Resort</h2>
        </div>
    </div>
</div>
  )
}

export default Booking